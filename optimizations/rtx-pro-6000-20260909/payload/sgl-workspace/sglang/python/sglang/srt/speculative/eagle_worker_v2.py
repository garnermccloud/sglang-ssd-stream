...
class _NvFp4A16DraftHeadMethod:
    def apply(self, layer, x, bias=None):
        ...
        if bias is not None:
            output = output + bias
        return output.view(*x.shape[:-1], layer.output_size_per_partition)


def _maybe_quantize_draft_lm_head(lm_head) -> None:
    mode = os.environ.get("SGLANG_SPECULATIVE_LM_HEAD_QUANT", "").strip().lower()
    if not mode:
        return
    if mode not in {"fp8", "nvfp4"}:
        raise ValueError(
            "SGLANG_SPECULATIVE_LM_HEAD_QUANT supports 'fp8' or 'nvfp4', "
            f"not {mode!r}"
        )
    ...
    if mode == "nvfp4":
        from flashinfer import SfLayout, nvfp4_quantize, prepare_bf16_fp4_weights
        from sglang.srt.layers.quantization.modelopt_quant import swizzle_blockscale

        source = lm_head.weight
        rows, columns = source.shape
        weight_amax = source.abs().nan_to_num_().amax().float()
        weight_scale = torch.where(
            weight_amax > 0,
            weight_amax / (448.0 * 6.0),
        ...
        packed, block_scale, alpha = prepare_bf16_fp4_weights(
            packed,
            swizzle_blockscale(
                block_scale.view(torch.float8_e4m3fn).reshape(rows, columns // 16)
            ),
            weight_scale.reshape(1),
            backend="cute-dsl",
        )
        del lm_head.weight
        lm_head.register_parameter("weight", Parameter(packed, requires_grad=False))
        lm_head.register_parameter(
            "weight_scale", Parameter(block_scale, requires_grad=False)
        )
        lm_head.register_parameter("alpha", Parameter(alpha, requires_grad=False))
        lm_head.logical_widths = [rows]
        lm_head.input_size_per_partition = columns
        lm_head.output_size_per_partition = rows
        lm_head.orig_dtype = source.dtype
        lm_head.quant_method = _NvFp4A16DraftHeadMethod()
        logger.info(
            "Quantized draft-only lm_head to NVFP4 W4A16: vocab=%d hidden=%d",
            rows,
            columns,
        )
        return
    ...
    from sglang.srt.layers.quantization.fp8_utils import per_token_group_quant_fp8

    source = lm_head.weight
    qweight, weight_scale = per_token_group_quant_fp8(source, source.shape[-1])
    del lm_head.weight
    lm_head.register_parameter(
        "weight", Parameter(qweight.t(), requires_grad=False)
    )
    lm_head.register_parameter(
        "weight_scale", Parameter(weight_scale.t().contiguous(), requires_grad=False)
    )
    lm_head.register_parameter("input_scale", None)
    lm_head.logical_widths = [source.shape[0]]
    lm_head.input_size_per_partition = source.shape[1]
    lm_head.output_size_per_partition = source.shape[0]
    lm_head.orig_dtype = source.dtype
    lm_head.quant_method = Fp8LinearMethod(
        Fp8Config(is_checkpoint_fp8_serialized=False, activation_scheme="dynamic")
    )
    logger.info(
        "Quantized draft-only lm_head to FP8: vocab=%d hidden=%d",
        source.shape[0],
        source.shape[1],
    )
...
class EagleDraftWorker(EagleDraftWorkerBase):
    ...
    def init_lm_head(self):
        ...
        if self.speculative_algorithm.is_eagle3():
            ...
            if self.hot_token_id is not None:
                ...
                head = head.detach().index_select(0, self.hot_token_id)

            # Share the embedding and lm_head
            self.draft_runner.model.set_embed_and_head(embed, head)
            maybe_share_target_lm_head()
            _maybe_quantize_draft_lm_head(self.draft_runner.model.lm_head)
    ...
    def _configure_qsa_mtp_index_share(self) -> None:
        ...
        # All depth-specific graphs must read the most recent draft-extend,
        # including immediately after switching to a different depth.
        tail_width = max_speculative_num_draft_tokens()
        state = getattr(self, "_qsa_mtp_shared_indices", None)
        if state is None:
            ...
            self._qsa_mtp_shared_indices = state
        elif state.tail_width != tail_width:
            raise RuntimeError("QSA shared selection capacity changed after graph capture")
    ...
    def _capture_cuda_graphs(self):
        ...
        if (
            ...
            if _is_cuda:
                assert self.cuda_graph_runner_for_draft_extend.captured_req_width == self.speculative_num_draft_tokens
